...
exports.getAllStaffData = async (req, res, next) => {
  try {
    // Fetch all staff data with the specified fields
...
exports.searchStaff = async (req, res, next) => {
  try {
    ...
    const { faculty, role, name } = req.query;

    // Build the query based on the provided parameters
    const query = {};

    if (faculty) {
      query.faculty = faculty; // Assuming faculty is an ObjectId
    }

    if (role) {
      query.roles = role; // Assuming roles is an array of strings, adjust accordingly
    }
    ...
    console.log(query);
    // Execute the query to find a single staff member
    const foundStaffMember = await Staff.findOne(query);

    // Send the result as a response
    res.status(200).json({
      status: "success",
      data: foundStaffMember ? [foundStaffMember] : [], // Wrap the result in an array for consistency
    });
  } catch (error) {
    // Handle errors
    res.status(500).json({
