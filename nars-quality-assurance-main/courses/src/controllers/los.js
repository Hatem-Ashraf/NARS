...
exports.createLos = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({
        error: "An error occurred while creating the Learning Objective",
      });
  }
};
...
exports.deleteLos = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({
        error: "An error occurred while deleting the Learning Objective",
      });
  }
};
...
exports.getAllLos = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        error: "An error occurred while fetching the Learning Objectives",
      });
  }
};
  ...
  try {
    ...
    if (!los || los.length === 0) {
      return res
        .status(404)
        .json({
          message: "No learning objectives found for the specified IDs.",
        });
    }
...
exports.getAllLosByDomain = async (req, res) => {
  ...
  if (!domain) {
    return res
      .status(400)
      .json({ message: "Missing required query parameter: domain" });
  }
...
exports.createMultipleLos = async (req, res) => {
  try {
    ...
    if (!LOs || !Array.isArray(LOs) || !courseId) {
      return res
        .status(400)
        .json({ error: "All fields are required (courseId, LOs array)" });
    }
    ...
    res
      .status(500)
      .json({
        error: "An error occurred while creating the Learning Objectives",
      });
  }
};
