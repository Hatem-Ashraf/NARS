...
const addStaff = ({ cookies }) => {
  ...
  const handleFacultyChange = async () => {
    ...
    const resp = await fetch(`http://localhost:8083/${selectedFacultyId}`, {
      headers: {
  ...
  let failImport = (
    ...
    >
      <i class="fa-sharp fa-solid fa-circle-exclamation"></i>
      <div class="ml-3 text-sm font-medium">
        Imported data is not in the right format please download the template
  ...
  let partiallyfailImport = (
    ...
    >
      <i class="fa-sharp fa-solid fa-circle-exclamation"></i>
      <div class="ml-3 text-sm font-medium">
        Parts of the imported data is not in the right format please download
        the template and follow the format.
        <a href="#" class="font-semibold underline hover:no-underline"></a>.
      </div>
      <button
        type="button"
