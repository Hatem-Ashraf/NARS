    ...
    const get_comp = async (e) => {
      ...
      try {
        ...
        comp1.forEach((elm) => {
          ...
          compTemp[elm.code]["avg"] =
            (elm.avg + compTemp[elm.code]["indirect"]) / 2;
        });
        courses.forEach((elm) => {
          coursesTemp[elm.name]["indirect"] = elm.avg;
          coursesTemp[elm.name]["avg"] =
            (elm.avg + coursesTemp[elm.name]["indirect"]) / 2;
        });
        setComp(compTemp);
        ...
        console.log("DATAAAAAA", JSON.stringify(data3));
      ...
      {1 && (
        <div className="flex flex-col w-full items-start">
          ...
          <div className="flex flex-col w-full items-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-1 md:gap-2 gap-y-1 w-[90%] mb-10">
              <div>
                <CoursesBar courses={coursesAvg} w={60} h={60} />
              </div>
              <div>
                <CompBar comp={comp} w={60} h={60} />
              </div>
            </div>
