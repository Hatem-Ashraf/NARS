...
const SearchStudent = ({ cookies }) => {
  ...
  const deleteHandler = async (e) => {
    ...
    try {
      const resp = await fetch(
        `http://localhost:8081/staff/${tobeDeleted._id}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
  ...
  const editHandler = async (e) => {
    ...
    try {
      const resp = await fetch(
        `http://localhost:8081/staff/${tobeEdited._id}`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
  ...
  const submitHandler = async (e) => {
    ...
    try {
      let url = `http://localhost:8081/staff?roles=${role.current.value}&faculty=${faculty.current.value}`;
      if (choosenRole == "any") {
        url = `http://localhost:8081/staff?faculty=${faculty.current.value}`;
      }
      ...
      >
        ...
        >
          <div className="overflow-auto contentAddUser2 flex flex-col gap-10">
            <div className="text-3xl font-bold text-blue-800 mb-6 mt-4">Search Staff</div>
