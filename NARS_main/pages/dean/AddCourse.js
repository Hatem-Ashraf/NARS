...
const CreateCourse = ({ cookies }) => {
  ...
  const handleAssign = (user) => {
    // Handle assigning the instructor here
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      ...
      <div className=" p-20 max-w-3xl bg-gray-100 w-full shadow-2xl rounded-3xl">
        <h2 className="font-bold text-form mb-4 text-3xl text-center">Assign a Course</h2>
        <form onSubmit={submitHandler}>
          <div className="flex flex-col gap-4 ">
          <label htmlFor="title" className="text-form font-bold w-1/3">Name:</label>
          <input
              type="text"
              id="title"
              name="title"
            ...
            <input
              ...
              id="hours"
              name="hours"
              value={userState.email} // Assuming email is accessible from userState
              onChange={handleChange}
              className="input-field"
              required
            />
